...
use conversion::{convert_unencrypted, ProgressSink};
use serde::Serialize;
...
struct ProgressEvent { path: String, state: String, progress: f32, detail: String, output_path: Option<String> }

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
struct FileSize { path: String, size: u64 }

#[tauri::command]
fn file_sizes(paths: Vec<String>) -> Vec<FileSize> {
    paths.into_iter().filter_map(|path| std::fs::metadata(&path).ok().map(|metadata| FileSize { path, size: metadata.len() })).collect()
}
...
async fn start_conversion(
    ...
    for path in requests {
        ...
        channel.send(event(&path, "converting", 2.0, "Checking CCI structure"))
            .map_err(|error| error.to_string())?;
        let mut progress = ChannelProgress { channel: channel.clone(), path: path.clone() };
        match convert_unencrypted(Path::new(&path), &output, &mut progress) {
            Ok(()) => channel.send(ProgressEvent { path: path.clone(), state: "completed".into(), progress: 100.0, detail: "Completed".into(), output_path: Some(output.display().to_string()) }),
...
struct ChannelProgress { channel: Channel<ProgressEvent>, path: String }

impl ProgressSink for ChannelProgress {
    fn report(&mut self, completed: u64, total: u64) {
        let ratio = if total == 0 { 0.0 } else { completed as f32 / total as f32 };
        let _ = self.channel.send(event(&self.path, "converting", 5.0 + ratio * 90.0, "Writing CIA content"));
    }
}

fn event(path: &str, state: &str, progress: f32, detail: &str) -> ProgressEvent {
    ProgressEvent { path: path.into(), state: state.into(), progress, detail: detail.into(), output_path: None }
}
...
fn output_for(input: &Path, mode: &str, shared_folder: Option<&str>) -> Result<PathBuf, String> {
    ...
    let folder = match mode {
        "source" | "same" => input.parent().ok_or_else(|| format!("{} has no parent folder", input.display()))?.to_path_buf(),
        "shared" => PathBuf::from(shared_folder.filter(|value| !value.trim().is_empty()).ok_or("Choose a shared output folder")?),
        _ => return Err("Unknown output mode".into()),
    };
    let output = folder.join(stem).with_extension("cia");
    if !output.exists() { return Ok(output); }
...
mod tests {
    ...
    fn resolves_a_shared_folder() {
        assert_eq!(
            output_for(Path::new("/games/Example.3ds"), "shared", Some("/converted")).unwrap(),
