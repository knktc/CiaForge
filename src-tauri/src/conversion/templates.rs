...
use super::{
    ConversionError,
    writer::{CERT_CHAIN_SIZE, TICKET_SIZE},
};
...
pub struct RetailTemplates {
    pub certificate_chain: Vec<u8>,
    pub ticket_and_tmd: Vec<u8>,
}
    ...
    pub fn load() -> Result<Self, ConversionError> {
        ...
        if certificate_chain.len() != CERT_CHAIN_SIZE {
            return Err(ConversionError::Template(
                "retail certificate chain has an unexpected length",
            ));
        }
        let ticket_and_tmd = decode(TICKET_TMD)?;
        if ticket_and_tmd.len() != TICKET_AND_TMD_TEMPLATE_SIZE
            || ticket_and_tmd.len() < TICKET_SIZE
        {
            return Err(ConversionError::Template(
                "retail ticket/TMD template has an unexpected length",
            ));
        }
        Ok(Self {
            certificate_chain,
...
fn decode(value: &str) -> Result<Vec<u8>, ConversionError> {
    let compressed = STANDARD
        .decode(value.trim())
        .map_err(|_| ConversionError::Template("invalid embedded base64"))?;
    let mut decoder = ZlibDecoder::new(compressed.as_slice());
    let mut decoded = Vec::new();
    decoder
        .read_to_end(&mut decoded)
        .map_err(|_| ConversionError::Template("invalid embedded zlib data"))?;
    Ok(decoded)
}
