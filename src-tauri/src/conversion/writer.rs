...
impl CiaHeader {
    pub fn new(plan: &CiaPlan, certificate_chain: &[u8], ticket_and_tmd: &[u8]) -> Result<Self, &'static str> {
        if certificate_chain.len() != CERT_CHAIN_SIZE { return Err("certificate chain must be 0xa00 bytes"); }
        ...
        if bytes.len() != CONTENT_RECORDS_OFFSET { return Err("CIA template does not align content records"); }
...
mod tests {
    use super::*;
    use crate::conversion::{cci::{EncryptionMode, Partition}, CciHeader, CiaPlan};

    #[test]
    fn serializes_a_fixed_size_cia_header() {
        let source = CciHeader { title_id: 1, game: Partition { offset: 0x400, size: 0x1000 }, manual: None, download_play_child: None, encryption: EncryptionMode::Unencrypted };
